...
def load_cleaned_dataframe():
    log_path = RESULT_DIR / "cleaning_output_log.txt"
...
def prepare_modeling_data(df_clean):
    missing_columns = [
...
def explain_best_model(best_result, X_train, X_test):
    import numpy as np
    import shap
    ...
    importance.to_csv(RESULT_DIR / "best_model_shap_importance.csv", index=False)

    plt.figure()
    shap.summary_plot(shap_values, shap_plot_data, plot_type="bar", show=False)
    plt.tight_layout()
    plt.savefig(RESULT_DIR / "best_model_shap_bar.png", dpi=300, bbox_inches="tight")
    plt.close()

    plt.figure()
    shap.summary_plot(shap_values, shap_plot_data, show=False, max_display=20)
    plt.tight_layout()
    plt.savefig(RESULT_DIR / "best_model_shap_summary.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    model_df, X, y, info = prepare_modeling_data(df_clean)
    model_df.to_csv(RESULT_DIR / "modeling_dataset_dropna.csv", index=False)
    ...
    results_df.to_csv(RESULT_DIR / "model_performance.csv", index=False)

    best_result = max(results, key=lambda result: (result["roc_auc"], result["f1"]))
    shap_importance = explain_best_model(best_result, X_train, X_test)
    ...
    summary = "\n".join(summary_lines)
    (RESULT_DIR / "analysis_summary.txt").write_text(summary, encoding="utf-8")
    print(summary)
